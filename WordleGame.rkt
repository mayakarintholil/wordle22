...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw9-problem1 (1) (1) (4)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (string-in-list? s los)
  (value-in-list?/new s los string=?))



; value-in-list?/new : (X) X [List-of X] [X X -> Boolean] -> Boolean
; is the supplied value in the list according to the supplied predicate?

(check-expect (value-in-list?/new 1 (list) =) #f)
(check-expect (value-in-list?/new "a" (list) string=?) #f)
(check-expect (value-in-list?/new 1 (list 1 2 3) =) #t)
(check-expect (value-in-list?/new 5 (list 1 2 3) =) #f)
(check-expect (value-in-list?/new "a" (list "a" "b" "c") string=?) #t)
(check-expect (value-in-list?/new "A" (list "a" "b" "c") string-ci=?) #t)

(define (value-in-list?/new x lox p?)
  (local [; apply-p? : Any -> Boolean
          ; given x, returns whether p? applies
          (define (apply-p? x2)
            (p? x x2))]
    (ormap apply-p? lox)))
...
(define (read-dictionary filename)
  (if (file-exists? filename)
      (map string-upcase (read-lines filename))
      '()))
...
(define (score guess correct)  
  (local [; give-lsp : 1String 1String -> LetterStatusPair
          ; given a character in the guess, returns the appropriate LSP
          ; after comparing to the correct answer
          (define (give-lsp guesscharacter correctcharacter)
            (make-pair guesscharacter (cond
                                        [(string=? correctcharacter guesscharacter) LS-RIGHT]
                                        [(string-in-list? guesscharacter (explode correct))
                                         LS-MISPLACED]
                                        [else LS-WRONG])))]
    (mymap2 (explode guess) (explode correct) give-lsp)))
...
(define (partial-guess->full-guess s n)
  (string-append s (implode (build-list (- n (length (explode s))) (λ (x) " ")))))
...
(define WS-WORD4 (make-wordle (list "CAT") "MAY"))
...
(define (key-wordle wordlestate ke los number)
  (cond [(key=? ke "\b") (backspace wordlestate)]
        [(key=? ke "\r") (return wordlestate los number)]
        [(letter? ke) (otherkey ke wordlestate number)]
        [else wordlestate]))

; backspace : WS -> WS
; Given a WordleState, processes the backspace key event to produce the previous WordleState

(check-expect (backspace WS-START) WS-START)
(check-expect (backspace WS-TYPE3) WS-TYPE2)
(check-expect (backspace WS-TYPE-BAD) WS-TYPE2)

(define (backspace ws)
  (if (= (string-length (wordle-current ws)) 0)
      ws
      (make-wordle (wordle-past ws)
                   (substring (wordle-current ws) 0
                              (- (string-length (wordle-current ws)) 1)))))

; return : WS [List-of String] Nat -> WS
; Given a WordleState, an LOS and a number, processes the return key event
; to produce the next WordleState

(check-expect (return WS-START LITTLE-WORDS 3) WS-START)
(check-expect (return WS-TYPE3 LITTLE-WORDS 3) WS-WORD1)
(check-expect (return WS-TYPE-BAD LITTLE-WORDS 3) WS-TYPE-BAD)

(define (return ws los number)
  (if (and (= (string-length (wordle-current ws)) number)
           (string-in-list? (wordle-current ws) los))
      (make-wordle (append (wordle-past ws) (list (wordle-current ws))) "")
      ws))

; letter? : 1String -> Boolean
; Given a 1String, checks whether it is in the supplied list of Strings  

(check-expect (letter? "A") #t)
(check-expect (letter? "d") #t)
(check-expect (letter? "3") #f)
(check-expect (letter? "!") #f)

(define (letter? l)
  (string-in-list? l (explode "abcdefghijklmnopqrstuvwyzABCDEFGHIJKLMNOPQRSTUVWXYZ")))

; otherkey : 1String WS Nat -> WS
; Given a 1String, WordleState and number, adds a letter to the current guess
; as long as it is shorter than the supplied number

(check-expect (otherkey "A" WS-TYPE1 3) WS-TYPE2)
(check-expect (otherkey "X" WS-TYPE2 3) WS-TYPE-BAD)
(check-expect (otherkey "c" WS-START 3) WS-TYPE1)
(check-expect (otherkey "C" WS-START 3) WS-TYPE1)

               
(define (otherkey o ws number)
  (if (>= (string-length (wordle-current ws)) number)
      ws
      (make-wordle (wordle-past ws) (string-append (wordle-current ws) (string-upcase o)))))
...
(define (end-wordle? ws guess guesslimit)
  (cond
    [(= (length (wordle-past ws)) guesslimit) #t]
    [(string-in-list? guess (wordle-past ws)) #t]
    [else #f]))
...
(check-expect (draw-end-wordle WS-WORD1 "BAD" 3)
              (overlay/align "middle" "top"
                             (text "You lost!" 50 "black")
                             (overlay/align "middle" "middle"
                                            (text (string-append "It took you 1 guesses") 50 "black")
                                            (square 200 "solid" "white"))))

(check-expect (draw-end-wordle WS-WORD2 "CAT" 1)
              (overlay/align "middle" "top"
                             (text "You won!" 50 "black")
                             (overlay/align "middle" "middle"
                                            (text (string-append "It took you 2 guesses") 50 "black")
                                            (square 200 "solid" "white"))))



(define (draw-end-wordle ws winningword numguesses)
  (local [; won-or-lost : WS -> String
          ; Given a WS, returns whether the player won or lost
          (define (won-or-lost? ws)
            (if (string-in-list? winningword (wordle-past ws))
                "You won!"
                "You lost!"))]                      
    (overlay/align "middle" "middle"
                   (text (string-append "It took you "
                                        (number->string (length (wordle-past ws)))
                                        " guesses") 50 "black")
                   (overlay/align "middle" "top"
                                  (text (won-or-lost? ws) 50 "black")
                                  (square 200 "solid" "white")))))
...
(check-expect (draw-wordle WS-WORD3 "hat" 3)
              (overlay/align "middle" "top"
                             (above
                              (old-guesses "hat" WS-WORD3)
                              (stack/h (map guess-letter->image
                                            (explode (wordle-current WS-WORD3)))))
                             (background 3 (string-length "hat"))))
(check-expect (draw-wordle WS-WORD4 "mat" 5)
              (overlay/align "middle" "top"
                             (above
                              (old-guesses "mat" WS-WORD4)
                              (stack/h (map guess-letter->image
                                            (explode (wordle-current WS-WORD4)))))
                             (background 5 (string-length "mat"))))
(check-expect (draw-wordle WS-TYPE2 "cat" 2)
              (overlay/align "middle" "top"
                             (above
                              (old-guesses "cat" WS-TYPE2)
                              (stack/h (map guess-letter->image
                                            (explode (wordle-current WS-TYPE2)))))
                             (background 2 (string-length "cat"))))
(check-expect (draw-wordle WS-TYPE-BAD "maya" 5)
              (overlay/align "middle" "top"
                             (above
                              (old-guesses "maya" WS-TYPE-BAD)
                              (stack/h (map guess-letter->image
                                            (explode (wordle-current WS-TYPE-BAD)))))
                             (background 5 (string-length "maya"))))

                

(define (draw-wordle ws word guesses)
  (overlay/align "middle" "top"
                 (above (old-guesses word ws)
                        (stack/h (map guess-letter->image (explode (wordle-current ws)))))
                 (background guesses (string-length word))))
    
                   
; old-guesses: String WS -> Image
; Given a word and a wordle state, returns a visualization of previous guesses
(check-expect (old-guesses "hat" WS-WORD3) (stack/v
                                            (map stack/h
                                                 (words-score (wordle-past WS-WORD3) "hat"))))
(check-expect (old-guesses "maggie" WS-TYPE2) (stack/v
                                               (map stack/h
                                                    (words-score (wordle-past WS-TYPE2) "maggie"))))
(check-expect (old-guesses "rat" WS-TYPE-BAD) (stack/v
                                               (map stack/h
                                                    (words-score (wordle-past WS-TYPE-BAD) "rat"))))

(define (old-guesses word ws)
  (stack/v (map stack/h (words-score (wordle-past ws) word))))



; words-score : [List-of Strings] String -> [List-of Images]
; scores the guess list
;(check-expect (words-score (list "lover" "hover") "mover")             

(define (words-score guesslist s)
  (local [;one-word-score : String -> List-of LSP
          ;Given a word, scores it to make list of LSP
          (define (one-word-score guessword) 
            (score guessword s))
          ;vis-word : LSP -> Image
          ;Given a LSP, visualizes it
          (define (vis-word guessword) 
            (map lsp->image (one-word-score guessword)))]
    (map vis-word guesslist))) 


(define EMPTY-BOX
  (overlay (square 60 "solid" "white") (square 64 "solid" BORDER-COLOR)))

; background : Nat Nat -> Image
; given the number of guesses allowed and a number for the length of the word,
; produces an empty wordle board
(check-expect (background 5 6)
              (overlay (stack/v (make-list 5 (stack/h (make-list 6 EMPTY-BOX))))
                       (rectangle 400 (* 5 80) "solid" "white")))
(check-expect (background 4 (string-length "maya"))
              (overlay (stack/v (make-list 4 (stack/h (make-list 4 EMPTY-BOX))))
                       (rectangle 400 (* 4 80) "solid" "white")))

(define (background guesses number)
  (overlay (stack/v (make-list guesses (stack/h (make-list number EMPTY-BOX))))
           (rectangle 400 (* guesses 80) "solid" "white"))) 
